...
def preprocess_data(data):
    ...
    scaled_data = scaler.fit_transform(data[['timestamp', 'longitude', 'latitude', 'temperature', 'wind_speed', 'precipitation', 'solar_radiation']])
...
def train_xgboost(X_train, X_val, y_train, y_val, model_path='Model/XGB/xgboost_model.json'):
    model = XGBRegressor(objective='reg:squarederror', n_estimators=2000, learning_rate=0.1, random_state=42, EarlyStopping=5)
...
def train_mlp(X_train, X_val, y_train, y_val, model_path='Model/MLP/mlp_model.pkl'):
    model = MLPRegressor(hidden_layer_sizes=(100, 50), max_iter=2000, alpha=0.01, solver='adam', random_state=42, early_stopping=True, validation_fraction=0.2, n_iter_no_change=10, verbose=True)
...
def evaluate_model(model, X_test, y_test, model_type):
    if model_type == 'lstm':
        # Ensure correct shape for LSTM: (samples, sequence_length, num_features)
        ...
        # For non-LSTM models, flatten the input
...
# Main function
def main():
    file_path = 'Data/Enriched_Migration_Data.csv'
